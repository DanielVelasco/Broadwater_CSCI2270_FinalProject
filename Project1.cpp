...
void displayMenu()
{
    ...
    cout<<"7: Add Delivery Location"<<endl;
    cout<<"8: Quit"<<endl;
}
...
int main()
{
    ...
    while(flag)
    {
        ...
        if (input == 1)
        {
            node *x = Q->headOfDelivery; //run through list of nodes
            while (x != NULL)
            {
                cout<<x->name<<endl;
                x = x->n;
        ...
        if (input == 7)
        {
            ...
            string inputl;
            cout<<"Enter delivery location:"<<endl;
            getline(cin,inputl); //takes in deliver name and priority, and puts them in delivery inputd
            if(inputl.length()>0)
            {
                Q->addDeliveryLocation(inputl);
            }
        }
        if (input == 8)
        {
